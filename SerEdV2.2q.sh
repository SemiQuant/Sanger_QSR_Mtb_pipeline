...
#VQSRecal on merged VCF
	...
	#merge depth files
	#10 DP
	Rscript "$Script_dir/merge_coverage_files.R" "$SeqStats/$Name BWA_sorted_dedup_realigned_sorted_genome_coverage_10depthsites.txt" "$SeqStats/$Name SMALT_sorted_dedup_realigned_sorted_genome_coverage_10depthsites.txt" "$Name" "$SeqStats"
	#all DP
	Rscript "$Script_dir/merge_coverage_files.R" "$SeqStats/$Name BWA_sorted_dedup_realigned_sorted_genome_coverage.txt" "$SeqStats/$Name SMALT_sorted_dedup_realigned_sorted_genome_coverage.txt" "$Name" "$SeqStats"
...
echo "$(date) SpolPred Started" >> "$basic_log"
		...
		gunzip "$R1paired"
		spolfile="${R1//.fastq}"
		sh "$Script_dir/SpolPred.sh" "$spolfile" "$MData" "$Name" "$Programs"
